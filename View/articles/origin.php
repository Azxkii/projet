...
<head>
    ...
    <title>RSAZXKI || ORIGIN</title>
</head>
<?php if (isset($_SESSION['authenticated']) && $user->getIsAdmin() && $user->getIsVerify()) { ?>
    <h1><a href="/viewCreateArticle">Créer un article</a></h1>
<?php } ?>
<tbody>
<h1 style="font-size: 160%; margin-bottom: -8%;">ORIGIN</h1>
<div class="article_psn">
    <?php
    foreach($params['articlesOrigin'] as $article) {
        /* @var Article $article */ ?>
        <div class="block_psn">
            <div class="zoom">
                <p class="para"><?= $article->getTitle()?></p>
                <p><a href="/article_id/<?= $article->getId() ?>"><img id="img" src="/assets/img/<?= $article->getImg()?>.png" alt="" srcset=""></a></p>
                <p><?= $article->getPrix()?> €</p>
                <p>
            </div>
                <?php
